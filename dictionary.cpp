...
typedef struct node
{
 ...
 char m[20];   //ARRAY TO STORE MEMORY
 class node  *left;     //ARRAY TO STORE LEFT NODE
 class node * right;    //ARRAY TO STORE RIGHT NODE
...
public:
 ...
 void create();      // TO CREATE NODE
 void disp(node *);    //TO DISPLAY BST
 void insert(node * root,node *temp);    //TO INSERT NEW NODE
 int search(node *,char []);     //TO SEARCH  NODE     
 int update(node *,char []);      //TO UPDATE NODE
 node* del(node *,char []);      //TO DELETE NODE
 node * min(node *);
};
